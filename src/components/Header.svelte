...
<header
  class={`bg-[hsla(0,100%,100%,0.9)] dark:bg-neutral-950 py-2 lg:py-4 backdrop-blur-sm backdrop-saturate-200 z-50 shadow border-b w-full sticky top-0 transition-all duration-300 ease-in-out ${isScrollingUp ? "translate-y-0" : "-translate-y-full"}`}
>
  <!-- Desktop nav  -->
  ...
  <nav class="lg:hidden">
    ...
    {#if nav}
      <div
        class="flex flex-col w-full gap-2 pb-8 pt-2 absolute font-medium top-0 bg-white dark:bg-neutral-950 h-screen"
        transition:fly
      >
        <div
